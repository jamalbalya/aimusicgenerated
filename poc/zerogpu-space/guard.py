...
class RateLimiter:
    ...
    def check(self, caller: str) -> None:
        """Count one request against `caller`, or refuse it with a 429."""
        now = time.time()
        key = caller.lower()
        with self._lock:
...
def authorize_request(request):
    ...
    result. It answers with the key the request is counted against, and raises
