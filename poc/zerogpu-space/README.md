...
**This Space requires a Hugging Face sign-in, and no configuration lifts it.**
Every path that costs GPU time or hands back a result — `/queue/join`,
`/queue/data`, `/call/*`, `/run/*`, `/api/*` and `/file=*` — requires an
`Authorization: Bearer <hugging face token>` header. The token is verified
against Hugging Face on this side of the wire, and the verified username is then
checked against an allowlist. Nothing in the request body is ever read as
identity, and an anonymous request is refused with 401 before any handler runs.
...
| `RATE_LIMIT_REQUESTS` / `RATE_LIMIT_WINDOW_SECONDS` | Optional abuse brake per verified user, default 6 per hour. In process memory: it resets whenever the Space restarts, and it is not the ZeroGPU quota. |

`REQUIRE_HF_SIGN_IN=1` is set as a Space **variable**. It is documentation
rather than a switch: `guard.sign_in_required()` returns true unconditionally,
so setting it to `0` changes nothing and the Space says so in its startup log
rather than quietly opening the door. There is no supported way to allow
anonymous generation here.

Rate limiting counts a **verified username**, never an address. A brake keyed to
an IP would be a throttle standing in for a boundary: addresses are shared,
spoofable and rotated freely, so counting anonymous callers by one is not a way
...
`test_guard.py` covers the boundary with no dependencies (`python3
test_guard.py`), and `live_boundary_test.py` attacks a real Gradio server over
real HTTP — locally, or a deployed Space with `--base URL`.
...
ACE-Step 1.5 at a pinned commit under `vendor/`, records that commit in
`vendor/COMMIT.txt`, and pushes. It never pushes weights or audio.
